...
import { forceBattleResult, recoverParty, woundParty } from '../game/cheats/battle.ts';
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (shortcut.id === 'toggleOverlay') {
        ...
        void Promise.resolve(quickSave(slot))
          ...
          .catch(() => cheatState.pushToast(`Save slot ${slot} failed`, 'danger'));
        ...
        void Promise.resolve(quickLoad(slot))
          ...
          .catch(() => cheatState.pushToast(`Load slot ${slot} failed`, 'danger'));
