...
export function SwitchesPanel() {
  ...
  const rows = switchEntries().filter((entry) => {
    ...
    return normalizedQuery
      ? `${entry.id} ${entry.name}`.toLowerCase().includes(normalizedQuery)
      : true;
  });
  ...
  return (
    <section>
      <PanelHeader
        title="Switches"
        description="Toggle RPG Maker switches. Translation features are intentionally omitted."
      />
      <div className="mb-4 grid gap-2 md:grid-cols-[1fr_auto_auto_auto]">
        ...
        <label className="grid grid-flow-col auto-cols-max cursor-pointer items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 text-sm">
          <input
            ...
            type="checkbox"
            onChange={(event) => setHideNameless(event.target.checked)}
          />
        ...
        <table className="w-full border-collapse text-left text-sm">
          <thead className="text-xs tracking-[0.18em] text-rmc-aether uppercase">
            <tr>
              ...
              <th className="sticky top-0 bg-rmc-panel px-4 py-3 z-10">
                Value
              </th>
            </tr>
          </thead>
