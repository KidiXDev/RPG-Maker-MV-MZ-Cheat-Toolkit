  ...
  return (
    <section>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3">
        <input className="rounded-lg border border-white/10 bg-rmc-abyss px-3 py-1.5 font-rmc-mono text-rmc-mist outline-none transition focus:border-rmc-aether" type="number" value={x} onChange={(event) => setX(Number(event.target.value))} />
        <input className="rounded-lg border border-white/10 bg-rmc-abyss px-3 py-1.5 font-rmc-mono text-rmc-mist outline-none transition focus:border-rmc-aether" type="number" value={y} onChange={(event) => setY(Number(event.target.value))} />
        <label className="col-span-2 grid grid-flow-col auto-cols-max cursor-pointer items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 text-sm">
